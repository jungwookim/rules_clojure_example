(ns user
  ...
  (:require
    ...
    [jungwoo.bazel-example.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (jungwoo.bazel-example.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (jungwoo.bazel-example.config/system-config {:profile :test})
                                  (ig/prep)))))
